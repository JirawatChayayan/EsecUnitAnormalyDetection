...
void setup() {
  ...
  xTaskCreate(&disp_Task, "Disp_Task", 5120, NULL, 1, NULL);

  //xTaskCreate(&main_Task, "Main_Task", 10240, NULL, 1, NULL);
...
unsigned int calculate(String res)
{
    if(res == "")
    {
      return 0;
    ...
    for(int i =0; i<n; i++)
    {
      if(char_array[i] == ',')
      {
        unsigned int dataPeriode = atol(buff.c_str());
        if(dataPeriode < 2000)
        {
          sumdata += dataPeriode;
          idx +=1;
    ...
    if(sumdata != 0 && idx != 0)
    {
       unsigned int T_avg = (sumdata)/(idx);
       return T_avg;
    }
    else
    {
